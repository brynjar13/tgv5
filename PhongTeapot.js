...
onload = function init()  {
    ...
    window.addEventListener("keydown", function (e) {
        var step = 0.1; // You can adjust this value for the step size
    
        switch (e.key) {
            case "ArrowRight":
                lightPosition[0] += step; // Move the light to the right on the x-axis
                break;
            case "ArrowLeft":
                lightPosition[0] -= step; // Move the light to the left on the x-axis
                break;
            case "ArrowUp":
                lightPosition[1] += step; // Move the light up on the y-axis
                break;
            case "ArrowDown":
                lightPosition[1] -= step; // Move the light down on the y-axis
                break;
            case "1":
            case "2":
            case "3":
            ...
            case "8":
            case "9":
                let NewTeapot = teapot(parseInt(e.key));
                points = NewTeapot.TriangleVertices;
                normals = NewTeapot.Normals;
                gl.bindBuffer( gl.ARRAY_BUFFER, vBuffer);
                gl.bufferData( gl.ARRAY_BUFFER, flatten(points), gl.STATIC_DRAW );
                gl.bindBuffer( gl.ARRAY_BUFFER, nBuffer);
                gl.bufferData( gl.ARRAY_BUFFER, flatten(normals), gl.STATIC_DRAW );
